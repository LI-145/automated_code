...
@ddt
class VerifyAuthTestCase(unittest.TestCase):
    """实名认证接口"""
    excel = ReadExcel(os.path.join(DATA_DIR, 'cases.xlsx'), 'auth')
    ...
    def setUp(self):
        ...
        log.info("请求数据：{}".format(data))
        response = self.web_s.requests(url=url, interface="userRegister", data=data)
        log.info("请求返回结果：{}".format(response))
    ...
    def test_case_verify_auth(self, case):
        ...
        response = self.web_s.requests(url=url, interface="verifyUserAuth", data=eval(case.data))
        log.info("请求返回结果：{}".format(response))
